 ...
 *    them. Every `.exhibit` — the container of everything the lab teaches — is
 *    a `linear-gradient(180deg, color-mix(in srgb, var(--bg-soft) 92%,
 *    transparent), var(--bg-soft))`, so axe cannot resolve the surface under a
 *    single word of prose in any of the six sections. `.objective`,
 *    `.callout-compare` and `.heuristic-note` are `color-mix()` tints of
 *    `--cyan`/`--amber` over that gradient; `.challenge`, `.embed-viz`,
 *    `details.selfcheck` and `.det-tip` are `color-mix()`es of `--bg-soft` or
 *    `--panel` toward `transparent`; and `body` itself carries two
 *    `radial-gradient` washes (cyan at 0% 0%, red at 100% 40%) over `--bg`.
 *    axe files all of that under `incomplete`, which means a violations-only
 *    gate measured the contrast of almost nothing on this page.
 ...
 *    at `.85` and `.embed-controls .btn:disabled` at `.55`, which is the state
 *    "Show the short vector emerge" ships in.
 *
 ...
 *    boxes are `#lll-log` (`.log`, `max-height: 420px; overflow: auto`, which
 *    grows a line per LLL step and overflows on any real reduction), every
 *    `pre.panel` / `pre.mono` (`overflow-x: auto` — the Gram-Schmidt dump, the
 *    LLL metrics block, the LWE attack transcript and both parameter-explorer
 *    columns), and `.embed-grid-wrap` (`overflow-x: auto` around a matrix that
 *    is wider than a phone). Content scrolled past those is not dimmed or
 ...
 *    draws all five of its figures into `<canvas>`, not SVG, so the sibling
 *    walk finds nothing to do on this page today. It is kept because the
 *    alternative is a helper that silently mismeasures the first labelled SVG
 *    figure anyone adds — an ancestor-only walk would composite a `<text>` onto
 *    the `<svg>`'s own transparent background rather than onto the `<rect>`
 *    drawn under it, and report a ratio nothing on screen has.
 *
 ...
 * rule. `e2e/nontext.ts` covers that gap with its own oracle, and `scan()`
 * asserts on it; this file does not.
 *
 * SECOND, `<canvas>` pixels. Five canvases carry the whole visual argument of
 * this lab — the lattice scene, the Gram-Schmidt scene, the LLL basis
 * evolution, the orthogonality-defect chart and the threshold chart — and
 * nothing here or in axe can read a pixel of them. That is a real and bounded
 * gap: each canvas has a text alternative beside it (a `role="img"`
 * `aria-label` that is rewritten with the live determinant, plus a
 * `.canvas-legend` naming every colour in words), and the numeric content each
 * one plots is also printed as text in `#lll-metrics`, `#gso-text` and
 * `#explore-left`/`#explore-right`, all of which ARE measured here.
 */
 ...
 * uses. The only genuinely short-lived styling here is `.embed-subtracting` on
 * the row the cancellation is currently subtracting, which the collapse
 * animation adds and removes around a 520ms wait — and that wait is
 * `prefersReducedMotion ? 0 : 520` in `main.ts`, so under the reduced motion
 * this gate asserts it is zero and the class never survives a frame. The
 * PERSISTENT end state of that animation (`.emerge-hot` on the secret block,
 * `.embed-resultrow-done` on the result row) is what the drive scans instead.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page: every `.exhibit` is a `linear-gradient(180deg, color-mix(in srgb,
     * var(--bg-soft) 92%, transparent), var(--bg-soft))` running the full
     * height of a section that can be several screens tall, so text near the
     * top of an exhibit sits on a different surface than text near its bottom.
     ...
     * is unmeasurable and paints nothing. The radial branch is load-bearing
     * here too: `body` carries two `radial-gradient(… at 0% 0% / 100% 40%)`
     * washes, one cyan and one red, and every panel above them is a
     * `color-mix(… , transparent)`, so both show through to the text.
     */
     ...
     *
     * A driven pass here walks five exhibits, six challenge cards and — the
     * expensive part — the embedding grid, which is rendered one `<td>` per
     * matrix entry and reaches 13 columns by 13 rows plus a live result row, so
     * a single instance is hundreds of sibling cells all re-walking the same
     ...
     * that is `#lll-log` (`.log`, `overflow: auto` under a 420px cap), every
     * `pre.panel` / `pre.mono` (`overflow-x: auto`), and `.embed-grid-wrap`
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for. Every figure this lab draws
      // itself is a <canvas>, which has no DOM inside it at all.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details class="selfcheck">` (five "Can you answer this now?" panels
      // and six challenge answers) plus `<details class="model-note">`, all
      // twelve of which ship shut. The gate opens each by clicking its
      // <summary>, which is the route a reader has, rather than setting `.open`
      // from script — which is what the gate this replaces did to all of them
      // at once, before its single scan.
      ...
      // `position:absolute; width:1px; height:1px; clip:rect(0,0,0,0)`, and it
      // is what carries the accessible names of the four Gram-Schmidt matrix
      // cells (`<label class="sr-only" for="gso-a11">` and its three
      // siblings), the LLL matrix textarea's label, and the `<caption>` of the
      // embedding grid. Every one of those has a rect and passes
      // `checkVisibility()` while painting exactly zero pixels; measuring them
      // reports ratios for ink that was never laid down. It is deliberately
      // narrow: only a ZERO-AREA clip qualifies, so a partially clipped
      // element is still measured, worst case.
     ...
     *
     * What this lab hides was enumerated rather than assumed, and it is a
     * closed set of two shapes:
     *
     *  - `.lwe-meter-track`, the bar behind `#lwe-meter-fill`. It owns no text
     *    and the percentage it depicts is written out in words immediately
     *    above it in `#lwe-meter-text`, inside a `role="status"` region.
     *  - the three `.canvas-legend` paragraphs. These DO own text, and marking
     *    them `aria-hidden` was a defect this sweep fixed — see the gate's
     *    header. They are now exposed, so this branch no longer skips them and
     *    every `.lg` chip in them is measured for real.
     *
     * Nothing on this page hides a VALUE behind `aria-hidden`. No determinant,
     * no metric, no attack verdict and no recovered secret is inside such a
     * subtree — which was checked rather than assumed, because that is the
     ...
     *
     * This lab has no SVG of its own — every figure it draws goes into a
     * `<canvas>`. The guard stays because the usual way the mistake arrives is
     * exactly how this lab builds its DOM: `innerHTML` template literals, used
     * for the whole page and again for every row of the embedding grid.
     ...
     * inside a `<g>` those render nothing while a walk that measured them would
     ...
     *
     * It is NOT a no-op here, and that is not hypothetical either: this
     * stylesheet already sets `html, body { min-height: 100% }`, the near
     * relative of the exact declaration that caused the fleet failure above,
     * and it paints the canvas from `body` — two `radial-gradient` washes over
     * `--bg`, which `getComputedStyle` reports on `body` while
     * `document.documentElement` reports `rgba(0,0,0,0)`. Without the
     * root-then-body fallback below, every element whose ancestors are all
     * transparent falls through to WHITE, and this lab's dark theme is `#0a0a14`.
     */
