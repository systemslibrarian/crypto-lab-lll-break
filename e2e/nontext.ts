...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * var(--bg-soft) 92%, transparent), var(--bg-soft))` spanning a section
     * that can be several screens tall, so a button near the top of an exhibit
     * sits on a different surface than one near its bottom. So each gradient is
     * *sampled* at the element's real location instead: linear by projecting
     ...
     * too: `body` carries two `radial-gradient` washes, cyan at 0% 0% and red
     * at 100% 40%, and every panel above them is a `color-mix(…, transparent)`,
     * so both show through to the control.
     */
     ...
     * what a page paints. It is live on this page: `body` paints its dark
     * ground with two `radial-gradient` washes over `--bg`, and a walk that
     * reads only `background-color` finds nothing opaque, falls through to
     * WHITE, and reports every control against a white page that does not
     * exist. The tell, elsewhere in this fleet, was a ratio that came out
     * IDENTICAL in both themes — a number that cannot depend on the theme is
     * not measuring the theme. Same class of mistake as the canvas-background
     * bug in `contrast.ts`, so it borrows that file's gradient-aware sampler
    ...
    const CONTROL = [
      ...
      '[role=radio]',
      '[role=slider]',
      // A LINK STYLED AS A BUTTON is a UI component under 1.4.11, so the list
      // covers that shape too. Restricted to link-shaped BUTTONS — prose links
      // identify themselves by their text and are not 1.4.11 cases — and
      // anything that slips through still has to pass the "is it trying to draw
      // itself as a control?" test below. Nothing inside `#app` matches it
      // today; the lab's only links are prose (`.heuristic-note a` and the
      // footer's related-demo list).
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');

    // Scoped to `#app`. The shared top bar is not this lab's to change — every
    // repo in the fleet carries a byte-identical copy inside `index.html` — and
    // its `.cl-btn` boundary is `color-mix(in srgb, var(--accent, #35d6bb) 38%,
    // transparent)` over the bar's fixed `#0b1512`, which measures 2.47:1 in
    // BOTH themes (the bar is always dark, so the page theme cannot move it).
    // That is a real 1.4.11 shortfall and it is reported upward as a fleet-wide
    // observation rather than patched in one repo; writing the exclusion down
    // here makes it a decision instead of an oversight. Everything the lab
    // itself renders lives under `#app` and is judged.
    const appRoot = document.getElementById('app') ?? document.body;
    for (const el of Array.from(appRoot.querySelectorAll(CONTROL))) {
